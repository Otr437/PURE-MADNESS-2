...
/// Runs the full ReAct harness suite, prints results, writes a JSON report.
/// Returns Ok(()) if all tests passed, Err if any test failed (after printing
/// the full report) — call this from a binary's #[tokio::main] async fn main().
pub async fn run_harness() -> Result<()> {
    ...
    if passed < results.len() {
        anyhow::bail!("{}/{} harness tests failed", results.len() - passed, results.len());
    }
    Ok(())
}
