...
use rag_ai_rust::react_loop::{run_react, ReactOptions, ToolRegistry, Step};
...
fn validate_env() {
    let provider = env::var("MODEL_PROVIDER").unwrap_or_else(|_| "anthropic".to_string());
    let provider = provider.trim().to_lowercase();
    let (env_var, display) = match provider.as_str() {
        "anthropic"        => ("ANTHROPIC_API_KEY", "anthropic"),
        "deepseek"         => ("DEEPSEEK_API_KEY",  "deepseek"),
        "deepseek-openai"  => ("DEEPSEEK_API_KEY",  "deepseek-openai"),
        "openai"           => ("OPENAI_API_KEY",     "openai"),
        other => {
            eprintln!("[harness] ERROR: unknown MODEL_PROVIDER '{}'", other);
            std::process::exit(1);
        }
    };
    let key = env::var(env_var).unwrap_or_default();
    if key.trim().is_empty() {
        eprintln!("[harness] ERROR: {} is not set (required for provider '{}')", env_var, display);
        std::process::exit(1);
    }
    eprintln!("[harness] provider={} model={}", display, env::var("MODEL_NAME").unwrap_or_default());
}
