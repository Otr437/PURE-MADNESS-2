...
router.post("/:id/approve", async (req: Request, res: Response) => {
  ...
  if (task.task_type === "email_draft") {
    ...
    try {
      await sendEmail({
        ...
        taskId:  draft.source_task_id ?? String(req.params.id),
        agentId: draft.agent_id ?? null,
      });

      logEntry(
        String(req.params.id), null,
      ...
      queries.updateTaskStatus.run("failed", err.message, "failed", "failed", String(req.params.id));
      logEntry(String(req.params.id), null, `Email send failed after approval: ${err.message}`, "error", "task");
      broadcast("TASK_UPDATED", { id: String(req.params.id), status: "failed", result: err.message });
  ...
  queries.resumeCheckpoint.run(note, String(req.params.id));

  logEntry(String(req.params.id), null, `Checkpoint approved by ${approver}: ${note}`, "info", "task");
...
router.post("/:id/reject", (req: Request, res: Response) => {
  ...
  queries.updateTaskStatus.run("failed", reason, "failed", "failed", String(req.params.id));

  logEntry(String(req.params.id), null, `Checkpoint rejected by ${(req as any).user?.username}: ${reason}`, "warning", "task");
  broadcast("TASK_UPDATED", { id: String(req.params.id), status: "failed", result: reason });
