...
router.get("/episodic/:taskId", (req: Request, res: Response) => {
  const rows = queries.getEpisodicByTask.all(String(req.params.taskId)) as any[];
  return res.json(rows);
});

router.get("/episodic/agent/:agentId", (req: Request, res: Response) => {
  const limit = Math.min(parseInt(String(req.query.limit ?? "50"), 10), 200);
  const rows  = queries.getEpisodicByAgent.all(String(req.params.agentId), limit) as any[];
  return res.json(rows);
});
...
router.get("/longterm/:agentId", (req: Request, res: Response) => {
  const rows = queries.getLongTermByAgent.all(String(req.params.agentId)) as any[];
  return res.json(rows);
});

router.delete("/longterm/:agentId", (req: Request, res: Response) => {
  queries.deleteLongTermByAgent.run(String(req.params.agentId));
  logEntry(null, String(req.params.agentId), `Long-term memory cleared by ${(req as any).user?.username}`, "warning", "agent");
