...
import "./tools/tradingBotControl.js";
...
export async function runReActLoop(task: ReActTask, agent: any): Promise<string> {
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    // Execute the tool — must be both registered AND explicitly allowed
    // for this agent's specialty. Looking it up in the global registry
    // alone is not sufficient: that would let any agent type invoke any
    // tool by name regardless of its specialty's allowedTools list.
    const isAllowedForThisAgent = tools.some(t => t.name === parsed.action);
    const tool = isAllowedForThisAgent ? getTool(parsed.action) : undefined;
    let observation: string;

    if (!isAllowedForThisAgent) {
      observation = `Error: Tool "${parsed.action}" is not available to this agent. Available tools: ${tools.map(t => t.name).join(", ")}`;
      consecutiveFailures++;
