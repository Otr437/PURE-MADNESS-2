...
router.post("/:id/approve", (req: Request, res: Response) => {
  ...
  queries.approveTask.run(req.params.id);
  logEntry(req.params.id, null, `Task approved by ${req.user?.username}`, "info", "task");
...
router.post("/:id/cancel", (req: Request, res: Response) => {
  ...
  queries.updateTaskStatus.run("failed", "Cancelled by user", "failed", "failed", req.params.id);
  logEntry(req.params.id, null, `Task cancelled by ${req.user?.username}`, "warning", "task");
  broadcast("TASK_UPDATED", { id: req.params.id, status: "failed", result: "Cancelled by user" });
  return res.json({ success: true });
});

// GET /api/tasks/:id/logs
router.get("/:id/logs", (req: Request, res: Response) => {
  const logs = queries.getLogsByTask.all(req.params.id) as any[];
  return res.json(logs);
});

// GET /api/tasks/:id/subtasks
router.get("/:id/subtasks", (req: Request, res: Response) => {
  const subTasks = queries.getSubTasksByTask.all(req.params.id) as any[];
