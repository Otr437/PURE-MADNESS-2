...
router.get("/:key", requireAdmin, (req: Request, res: Response) => {
  const row = queries.getSetting.get(req.params.key) as any;
...
router.put("/:key", requireAdmin, (req: Request, res: Response) => {
  ...
  const saved = queries.getSetting.get(req.params.key) as any;
  const sanitized = sanitizeSetting(saved);

  logEntry(null, null, `Setting "${req.params.key}" updated by ${req.user?.username}`, "info", "system");
...
router.delete("/:key", requireAdmin, (req: Request, res: Response) => {
  ...
  broadcast("SETTING_DELETED", { key: req.params.key });
