...
function validateEnv() {
  ...
  if (!key) {
    process.stderr.write("[harness] ERROR: ANTHROPIC_API_KEY is not set\n");
    process.exit(1);
  }
  if (!key.startsWith("sk-ant-")) {
    process.stderr.write("[harness] WARN: ANTHROPIC_API_KEY does not look like a valid Anthropic key\n");
  }
}
