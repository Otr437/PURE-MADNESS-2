...
public class ReactLoop {
    ...
    // ── API call with retry ───────────────────────────────────────────────────
    static JsonNode callApi(ArrayNode messages, String system) throws Exception {
        ...
        for (int attempt = 0; attempt <= backoff.length; attempt++) {
            Request request = new Request.Builder()
                ...
                .post(RequestBody.create(JSON.writeValueAsBytes(body),
            ...
            try (Response response = HTTP.newCall(request).execute()) {
                ...
                if (response.code() == 429 || response.code() >= 500) {
                    lastError = new RuntimeException("API " + response.code() + ": " + responseBody);
                    if (attempt < backoff.length) {
                        System.err.printf("[react] rate limit / server error, retrying in %ds...%n", backoff[attempt]);
                        Thread.sleep(backoff[attempt] * 1000L);
                        continue;
                    }
                } else {
                    throw new RuntimeException("API error " + response.code() + ": " + responseBody);
                }
            }
        }
        throw lastError;
